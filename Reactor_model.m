...
lambda_neutron = 0.5; %Lambda pour les fuites de neutrons et barres de controle
T_PF = 1;
...
N(1,1) = flux_thermal; %Flux initial [#/m^2.s]

for i = 2:length(T)
    Y(i,1) = Y(i-1,1) + (- Y(i-1,1)*fis_U235*N(i-1,1))*dt_gen; %U235
    Y(i,2) = Y(i-1,2) + (- Y(i-1,2)*fis_U238*N(i-1,1) - Y(i-1,2)*cap_U238*N(i-1,1))*dt_gen; %U238
    ...
    Y(i,6) = Y(i-1,6) + (Y(i-1,1)*fis_U235*N(i-1,1) + Y(i-1,2)*fis_U238*N(i-1,1) + Y(i-1,3)*fis_U239*N(i-1,1) + Y(i-1,4)*fis_Np239*N(i-1,1) + Y(i-1,5)*fis_Pu239*N(i-1,1))*2*dt_gen - Y(i-1,6)*lambda_PF*dt_gen; %PF*
    Y(i,7) = Y(i-1,7) + Y(i-1,6)*lambda_PF*dt_gen;
    
    N(i,1) = N(i-1,1) + (Y(i-1,1)*fis_U235*N(i-1,1) + Y(i-1,2)*fis_U238*N(i-1,1) - Y(i-1,2)*cap_U238*N(i-1,1) + Y(i-1,3)*fis_U239*N(i-1,1) + Y(i-1,4)*fis_Np239*N(i-1,1) + Y(i-1,5)*fis_Pu239*N(i-1,1) + Y(i-1,6)*lambda_PF)*NA*(v_thermal/V)*dt_gen - N(i-1,1)*lambda_neutron*dt_gen; %Flux
end
