...
for i = 2:length(T)
    ...
    if Power(i,1) < (P_min + 0.5)
        if ((Power(i,1) - Power(i-1,1))/Power(i-1,1))/dt_gen <= 0.05
            lambda_perte_th = lambda_perte_th * 0.9;
            lambda_perte_rap = lambda_perte_rap * 0.9;
        end
    elseif Power(i,1) >= (P_min + 0.5) && Power(i,1) <= P_max
        if ((Power(i,1) - Power(i-1,1))/Power(i-1,1))/dt_gen > 0
            lambda_perte_th = lambda_perte_th / 0.7;
            lambda_perte_rap = lambda_perte_rap / 0.7;
        elseif ((Power(i,1) - Power(i-1,1))/Power(i-1,1))/dt_gen < 0
            lambda_perte_th = lambda_perte_th * 0.7;
            lambda_perte_rap = lambda_perte_rap * 0.7;
        end   
    elseif Power(i,1) > P_max
        fprintf('\n WARNING : POWER > P_MAX : %f GW > %i GW. \n', Power(i,1), P_max);
    end
